...
@section('content')
    <div class="page-header">
        <div class="container">
            <nav class="breadcrumbs" aria-label="Хлебные крошки"><a href="{{ route('index') }}">Главная</a><span>/</span><a href="{{ route('news.allNews') }}">Новости</a><span>/</span><span>{{ $news->title }}</span></nav>
            <div class="article-meta"><span class="tag">{{ $news->tag->title }}</span><time datetime="{{ \Carbon\Carbon::parse($news->created_at)->translatedFormat('j F Y') }}">{{ \Carbon\Carbon::parse($news->created_at)->translatedFormat('j F Y') }}</time></div>
            <h1>{{ $news->title }}</h1>
        </div>
    </div>
    ...
    <section class="section">
        <div class="container">
            <div class="article-wrap">
                ...
                <a href="{{ route('news.allNews') }}" class="back-link">← Ко всем новостям</a>
    ...
    <section class="section section--soft" aria-labelledby="related-title">
        <div class="container">
            <div class="section-head">
                <div class="eyebrow">Ещё новости</div>
            </div>
            <div class="news-grid">
                @foreach($otherNews as $oN)
                    <article class="news-card">
                        <div class="news-thumb" aria-hidden="true">
                            <img src="{{ asset('img/news/' . $oN->img) }}" alt="">
                        </div>
                        <div class="news-body">
                            <span class="news-date">26 мая 2026</span>
                            <h3>{{ $oN->title }}</h3>
                            <a href="{{ route('news.oneNews', $oN) }}" class="read-more">Читать далее →</a>
                        </div>
                    </article>
                @endforeach
            </div>
          ...
          "@graph": [
            {
              ...
              "itemListElement": [
                ...
                {"@type":"ListItem","position":3,"name":"С 23 февраля!","item":"https://www.dttermo.ru/news/s-23-fevralja.html"}
              ]
            },
            {
              "@type": "NewsArticle",
              "@id": "https://www.dttermo.ru/news/s-23-fevralja.html",
              "headline": "С 23 февраля!",
...
@push('styles')
    <style>
        ...
        *:focus-visible{
        ...
        .article-meta{ position:relative; z-index:2; display:flex; align-items:center; gap:.8em; margin-bottom:.9rem; font-size:.85rem; color:#9BA39F; }
        .article-meta .tag{
            font-weight:700; font-size:.72rem; letter-spacing:.06em; text-transform:uppercase;
            color:#8FE0A6; background:rgba(69,168,97,.16); border:1px solid rgba(69,168,97,.4);
            padding:.3em .8em; border-radius:999px;
        }
        .page-header h1{ position:relative; z-index:2; color:#fff; margin-bottom:0; max-width:26ch; }

        /* ARTICLE */
        .article-wrap{ max-width:760px; margin-inline:auto; }
        .article-cover{
            aspect-ratio:21/9; border-radius:var(--radius-lg); background:var(--c-gray-light);
        ...
        .article-body ul{ display:grid; gap:.6rem; margin:0 0 1.4em; }
        .article-body ul li{ display:flex; gap:.7rem; align-items:flex-start; color:var(--c-ink-soft); }
        .article-body ul li::before{
            content:""; width:6px; height:6px; border-radius:50%; background:var(--c-green);
            margin-top:.6em; flex-shrink:0;
        }
        .article-body blockquote{
            margin:1.8em 0; padding:1.2em 1.5em; border-left:3px solid var(--c-green);
        ...
        .back-link{
            display:inline-flex; align-items:center; gap:.5em; font-weight:700; font-size:.9rem;
            color:var(--c-green-dark); margin-bottom:2rem;
        }
        .article-footer{
            display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem;
            margin-top:2.5rem; padding-top:1.8rem; border-top:1px solid var(--c-line);
        }
        .share-row{ display:flex; align-items:center; gap:.8rem; font-size:.85rem; color:var(--c-gray); }
        .share-row a{
        ...
        .share-row a:hover{ background:var(--c-green); border-color:var(--c-green); color:#fff; }
